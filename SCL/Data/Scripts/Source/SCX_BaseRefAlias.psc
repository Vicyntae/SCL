...
Event OnInit()
  ...
  If JC_Container
    ...
    ElseIf JValue.isIntegerMap(JC_Container)
      ...
      Else
        Issue("RefAlis " + GetName() + "has JC_List but lacks IntKey!", 1)
      EndIf
    EndIf
  Else
    Notice("JC_List not available for refalias " + GetName() + ".")
  EndIf
  JMap.setForm(SCLSet.JM_RefAliasList, _getStrKey(), GetOwningQuest())
...
Function _reloadMaintenence()
  ...
  Int StoredVersion = JDB.solveInt(".SCLExtraData.VersionRecords." + ScriptID)
  Int NewVersion = checkVersion(StoredVersion)
  JDB.solveIntSetter(".SCLExtraData.VersionRecords." + ScriptID, NewVersion, True)
  reloadMaintenence()
EndFunction
...
Int Function getTargetData(Actor akTarget, Bool abGenProfile = False)
  ...
  If Target
    ...
    If !Data && abGenProfile
      Bool Basic = False
      If akTarget == PlayerRef || akTarget.IsInFaction(SCXLib.PotentialFollowerFaction)
        Basic = True
      EndIf
...
Function getTeammates()
  ...
  while num && numFound<16
    ...
    if a && a.IsInFaction(SCXLib.CurrentFollowerFaction)
