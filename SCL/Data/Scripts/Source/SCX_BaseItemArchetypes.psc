...
Alias[] Property ItemTypeForms Auto  ;Stores base item types for fast retrieval
...
Int Function addToContents(Actor akTarget, ObjectReference akReference = None, Form akBaseObject = None, Int aiItemType, Int aiStoredItemType = 0, Float afWeightValueOverride = -1.0, Int aiItemCount = 1, Bool abMoveNow = True)
  Int TypeIndex = ItemTypes.find(aiItemType)
  Int ReturnEntry
  If TypeIndex != -1
    If ItemTypeForms.length != ItemTypes.length
      ItemTypeForms = Utility.CreateAliasArray(ItemTypes.length)
    EndIf
    SCX_BaseItemType TypeForm = ItemTypeForms[TypeIndex] as SCX_BaseItemType
    If !TypeForm
      TypeForm = SCXLib.getSCX_BaseAlias(SCXSet.JI_BaseItemTypes, aiItemType) as SCX_BaseItemType
      ItemTypeForms[TypeIndex] = TypeForm
    EndIf
    ReturnEntry = TypeForm.addToContents(akTarget, akReference, akBaseObject, aiStoredItemType, afWeightValueOverride, aiItemCount, abMoveNow)
  Else
    SCX_BaseItemType TypeForm = SCXLib.getSCX_BaseAlias(SCXSet.JI_BaseItemTypes, aiItemType) as SCX_BaseItemType
    If TypeForm
      ReturnEntry = TypeForm.addToContents(akTarget, akReference, akBaseObject, aiStoredItemType, afWeightValueOverride, aiItemCount, abMoveNow)
    EndIf
  EndIf
